package com.maple.fastdev.pagestatus
...
class PageStatusManager {
    ...
    fun setPageStatusChangeAction(action: PageChangeAction) {
        mPageStatusLayout.setPageStatusChangeAction(action)
    }
...
open class PageChangeAction {
    open fun onShowLoading(loadingView: View?) {
        val ivLoading: ImageView? = loadingView?.findViewById(R.id.iv_loading)
        val drawable = ivLoading?.background
